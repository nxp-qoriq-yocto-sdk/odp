...
pkt_disposition_e do_route_fwd_db(odp_packet_t pkt)
{
	...
	if (flow) {
		...
		if (fwd_entry) {
			...
			flow->l4_dport = dport;
			flow->l3_proto = proto;
			flow->out_port.queue = fwd_entry->queue;
...
do_opt:
	...
	odp_packet_user_ptr_set(pkt, flow->out_port.queue);
		...
		for (i = 0; i < num; i++) {
			...
			if (ODP_EVENT_PACKET == odp_event_type(ev[i])) {
				...
				if (rc)
					continue;
				odp_queue_enq_multi(
					(odp_queue_t)odp_packet_user_ptr(pkt),
					&ev[i], 1);
				rc = PKT_DONE;
...
main(int argc, char *argv[])
{
	...
	for (i = 0; i < num_workers; i++) {
		completionq[i] = queue_create("completion",
	...
	odph_linux_pthread_create(thread_tbl, &cpumask, pktio_thread, NULL);
