...
handle_ingress_packets(odpfsl_knidev_t kdev, unsigned *pktin)
{
	...
	for (i = 0; i < kdev_t_count; i++) {
		pkt_count = odp_pktio_recv(pktio, packets,
				KNID_MAX_PKT_BURST);
		if (pkt_count > 0) {
...
handle_egress_packets(odpfsl_knidev_t kdev, unsigned *pktout)
{
	...
	for (i = 0; i < kdev_t_count; i++) {
		...
		if (kni_rx_packets > 0) {
			...
			pkt_count = odp_pktio_send(pktio, packets, \
							kni_rx_packets);
			if (pkt_count <= 0) {
...
initialize_odp(void)
{
	...
	if (odp_init_local(ODP_THREAD_CONTROL)) {
		...
		odp_term_global();	/* Ignore error in this */
		exit(FAILURE);
	}
}
...
initialize_knidev_array(args_t *args)
{
	 ...
	 */
	pktio_params.in_mode = ODP_PKTIN_MODE_RECV;
	pktio_params.out_mode = ODP_PKTOUT_MODE_SEND;
	...
	for (i = 0; i < args->if_count; i++) {
		...
		/* Allocating KNI dev */
		ret = odpfsl_knidev_open(kdev_p, i, pktio, &kdev_ops);
		if (SUCCESS != ret) {
