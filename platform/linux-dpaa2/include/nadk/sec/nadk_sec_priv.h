...
/**
 * @file		nadk_sec_priv.h
 * @description	Private function & definitions for NADK Sec Device
 */

#ifndef _NADK_SEC_PRIV_H_
#define _NADK_SEC_PRIV_H_
...
struct nadk_sec_priv {
	/* More Info to be added */
	struct nadk_vq rx_vq[MAX_RX_VQS];
	struct nadk_vq tx_vq[MAX_TX_VQS];
};
...
struct nadk_per_thread_info {
	struct qbman_swp *swp; /*!< I/O handle for this thread,
				* for the use of NADK framework. This is
	...
	int32_t notification_eventfd; /*!< Eventfd registered for TX Error,
				...
				* filled by NADK and application will
...
/*!
 * @details	SEC API to register to NADK framework. It will be called
 *		by NADK and will register its device driver to NADK.
 *
...
/*!
 * @details	SEC API to unregister to NADK framework. It will be called
 *		by NADK and will unregister its device driver to NADK.
 *
 * @returns	NADK_SUCCESS on success; NADK_FAILURE otherwise.
 *
 */
 ...
 *
 * @param[in]	dev - Pointer to NADK SEC device
 *
 * @returns	NADK_SUCCESS on success; NADK_FAILURE otherwise.
 *
 */
int32_t nadk_sec_probe(struct nadk_dev *dev, const void *);
 ...
 * @param[in]	dev - Pointer to NADK SEC device
 *
 * @param[in]	vq_index - Pointer to NADK SEC device
 *
...
/*!
 * @details	Returns the nadk_dev SEC object
 *
 * @param[in]	none
 *
 * @return	pointer to SEC object of nadk_dev type
 */
