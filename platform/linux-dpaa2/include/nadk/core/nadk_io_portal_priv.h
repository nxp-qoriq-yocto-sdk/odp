...
/**
 * @file		nadk_io_portal_priv.h
 * @description		Private functions & MACRO definitions for NADK Data Path I/O portal
			type Device
 */

#ifndef _NADK_IO_PORTAL_PRIV_H_
#define _NADK_IO_PORTAL_PRIV_H_
...
#define NADK_INVALID_CHANNEL_IDX	((uint8_t)(-1))

/*Stashing Macros*/
#define NADK_CORE_CLUSTER_BASE		0x04
#define NADK_CORE_CLUSTER_FIRST		(NADK_CORE_CLUSTER_BASE + 0)
#define NADK_CORE_CLUSTER_SECOND	(NADK_CORE_CLUSTER_BASE + 1)
#define NADK_CORE_CLUSTER_THIRD		(NADK_CORE_CLUSTER_BASE + 2)
#define NADK_CORE_CLUSTER_FOURTH	(NADK_CORE_CLUSTER_BASE + 3)

#define NADK_CORE_CLUSTER_GET(sdest, cpu_id) \
do { \
	if (cpu_id == 0 || cpu_id == 1) \
		sdest = NADK_CORE_CLUSTER_FIRST; \
	else if (cpu_id == 2 || cpu_id == 3) \
		sdest = NADK_CORE_CLUSTER_SECOND; \
	else if (cpu_id == 4 || cpu_id == 5) \
		sdest = NADK_CORE_CLUSTER_THIRD; \
	else \
		sdest = NADK_CORE_CLUSTER_FOURTH; \
} while (0)

/*
 * The NADK DPIO device structure.
 */
struct nadk_dpio_dev {
	TAILQ_ENTRY(nadk_dpio_dev) next; /**< Pointer to Next device instance */
	struct qbman_result *hold_dqrr; /**< Last DQRR Entry which is on hold for this SW portal */
	nadk_mbuf_pt hold_buf; /**< Last buffer which is on hold  w.r.t hold_dqrr ptr */
 ...
 */
TAILQ_HEAD(nadk_dpio_device_list, nadk_dpio_dev); /*!< NADK DPIO device List */
extern struct nadk_dpio_device_list *dpio_dev_list; /*!< Global list of NADK devices. */
...
extern struct nadk_dpio_dev *notif_dpio;
 ...
 *
 * @param[in]	dev - Pointer to NADK device
 *
 ...
 *
 */
int32_t nadk_io_portal_probe(struct nadk_dev *dev,
 ...
 *
 */
int32_t nadk_io_portal_close(struct nadk_dev *dev);
