 ...
 * It is mandatory to pass NADK_MPOOL_NO_PRIVATE_MEMSET flag with this flag.
 */
#define NADK_MPOOL_NEED_PREPARE	BIT_POS(5)

/*!
 * An object constructor callback function for mempool.
 *
 * Arguments are the mempool, the opaque pointer given by the user in
 * nadk_mpool_create(), the pointer to the element and the index of
 * the element in the pool.
 */
typedef void (nadk_mpool_obj_ctor_t)(void *pool, void *obj_init_arg,
 ...
 *
 */
uint64_t nadk_mpool_size(void *mpool);
...
void_t *nadk_mpool_create(struct nadk_mpool_cfg *cfg,
	nadk_mpool_obj_ctor_t *obj_init, void *obj_init_arg);
 ...
 *
 */
int32_t nadk_mpool_getstats(
	void_t *mpool,
	struct nadk_mpool_stats *pstats);
 ...
 * @param[in]	mpool - pointer to memory pool
 *
 * @returns	Pointer to structure nadk_mpool_cfg; NULL on FAILURE.
 *
 */
struct nadk_mpool_cfg *nadk_mpool_getcfg(void_t *mpool);
 ...
 *
 */
int32_t nadk_mpool_set_threshold_vals(void_t *mpool,
 ...
 *
 */
void_t *nadk_mpool_from_blk(uint8_t *p_block);
 ...
 *
 */
int nadk_mpool_is_full(const void_t *mpool);
 ...
 *
 */
int nadk_mpool_is_empty(const void_t *mpool);
 ...
 *
 */
phys_addr_t nadk_mem_virt2phy(const void *p_virt);
 ...
 *
 */
phys_addr_t nadk_get_mpool_blk_phyaddr(
	const void_t *mpool,
	const void *blk);
 ...
 *
 */
void *nadk_get_mpool_blk_virtaddr(
	const void_t *mpool,
 ...
 *
 */
void nadk_mpool_dump(const void_t *mpool);
 ...
 *
 */
void nadk_mpool_list_dump(void *stream);
