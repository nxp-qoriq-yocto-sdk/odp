...
int cmdif_register_module(const char *m_name, struct cmdif_module_ops *ops)
{
	...
	if (odp_atomic_add_fetch_u16(&module_count, 1) == 1) {
		...
		if (ret != 0) {
			NADK_ERR(CMD, "CMDIF server Initalization failed");
			return ret;
		}
...
int cmdif_srv_cb(int pr, void *send_dev)
{
	...
	if (pkt_rcvd < 0) {
		NADK_ERR(APP1, "Error calling receive_fd");
		return NADK_FAILURE;
	}
	...
	if (err) {
		if (err == -EPERM)
			NADK_ERR(CMD, "Got command with invalid auth_id");
		else if (err == -EINVAL)
			NADK_ERR(CMD, "Invalid parameters for cmdif_srv_cmd");
		return err;
	}
	if (send_resp)
...
{
	int      err = 0;
	uint32_t dpci_id = get_aiop_dev_id((struct nadk_dev *)(send_dev));
