...
static int vfio_connect_container(struct vfio_group *vfio_group)
{
	...
	for (i = 0; i < VFIO_MAX_CONTAINERS; i++) {
		...
		if (!ioctl(vfio_group->fd, VFIO_GROUP_SET_CONTAINER, &container->fd)) {
			NADK_ERR(FW, "Container pre-exists with FD[0x%x]"
					" for this group\n", container->fd);
	...
	if (fd < 0) {
		NADK_ERR(FW, "vfio: error opening VFIO Container\n");
		return NADK_FAILURE;
	}
	...
	if (ioctl(fd, VFIO_CHECK_EXTENSION, VFIO_TYPE1_IOMMU)) {
		...
		if (ioctl(vfio_group->fd, VFIO_GROUP_SET_CONTAINER, &fd)) {
			NADK_ERR(FW, "VFIO_GROUP_SET_CONTAINER failed.\n");
			close(fd);
			return NADK_FAILURE;
		}
		/* Initialize SMMU */
		if (ioctl(fd, VFIO_SET_IOMMU, VFIO_TYPE1_IOMMU)) {
			NADK_ERR(FW, "VFIO_SET_IOMMU failed.\n");
			close(fd);
			return NADK_FAILURE;
		}
		NADK_INFO(FW, "VFIO_TYPE1_IOMMU Supported\n");
	} else {
		NADK_ERR(FW, "vfio error: No supported IOMMU\n");
		close(fd);
		return NADK_FAILURE;
	}
	...
	for (i = 0; i < VFIO_MAX_CONTAINERS; i++) {
		if (vfio_containers[i].used)
			continue;
		NADK_INFO(FW, "Found unused container at index %d\n", i);
		container = &vfio_containers[i];
	}
	if (!container) {
		NADK_ERR(FW, "vfio error: No Free Container Found\n");
		close(fd);
		return NADK_FAILURE;
	}
	container->used = 1;
	...
	NADK_INFO(FW, "Assigning Group to index group_list[%d]\n", container->index);
...
static void vfio_disconnect_container(struct vfio_group *group)
{
	...
	if (!container) {
		NADK_WARN(FW, "Invalid container");
		return;
	}

	if (ioctl(group->fd, VFIO_GROUP_UNSET_CONTAINER, &container->fd))
		NADK_ERR(FW, "UNSET Container API Failed with ERRNO = %d\n", errno);
...
static void vfio_unmap_irq_region(struct vfio_group *group)
{
	...
	if (ret)
		NADK_ERR(FW, "Error in vfio_dma_unmap (errno = %d)", errno);
}
...
static int vfio_map_irq_region(struct vfio_group *group)
{
	...
	if (vaddr == MAP_FAILED) {
		NADK_ERR(FW, "Error mapping GITS region (errno = %d)", errno);
		return -errno;
	}
	...
	NADK_ERR(FW, "vfio_map_irq_region fails (errno = %d)", errno);
	return -errno;
}
				...
				uint64_t size)
{
	...
	if (ioctl(group->container->fd, VFIO_IOMMU_MAP_DMA, &dma_map)) {
		NADK_ERR(FW, "SWP: VFIO_IOMMU_MAP_DMA API Error %d.\n", errno);
		return NADK_FAILURE;
	}
	return NADK_SUCCESS;
}
...
static int32_t setup_dmamap(void)
{
	...
	for (i = 0; i < NADK_MAX_MEMSEG; i++) {
		...
		if (memseg == NULL) {
			NADK_ERR(FW, "Cannot get physical layout\n");
			return -1;
		}

		if (memseg[i].addr == NULL && memseg[i].len == 0) {
			NADK_INFO(FW, "No more valid memsegs\n");
			break;
		}
		...
		NADK_INFO(FW, "Initial SHM Virtual ADDR %llX\n", dma_map.vaddr);
		NADK_INFO(FW, "DMA size 0x%lX\n", dma_map.size);
		dma_map.iova = (uint64_t)(NADK_VADDR_TO_IOVA(dma_map.vaddr));
		ret = ioctl(group->container->fd, VFIO_IOMMU_MAP_DMA, &dma_map);
		if (ret) {
			NADK_ERR(FW, "VFIO_IOMMU_MAP_DMA API Error %d.\n", errno);
			return NADK_FAILURE;
		}
		NADK_INFO(FW, "-----> dma_map.vaddr = 0x%llX\n", dma_map.vaddr);
	}
...
void destroy_dmamap(void)
{
	...
	for (i = 0; i < NADK_MAX_MEMSEG; i++) {
		...
		NADK_INFO(FW, "DMA-UNMAP size 0x%lX\n", dma_unmap.size);

		ret = ioctl(group->container->fd, VFIO_IOMMU_UNMAP_DMA, &dma_unmap);
		if (ret)
			NADK_ERR(FW, "VFIO_IOMMU_UNMAP_DMA API Error %d.\n", errno);
	}
...
static int vfio_set_group(struct vfio_group *group, int groupid)
{
	...
	if (group->fd < 0) {
		NADK_ERR(FW, "vfio: error opening %s\n", path);
		return NADK_FAILURE;
	}
	NADK_INFO(FW, "vfio: Open FD[0x%X] for IOMMU group = %s\n",
		group->fd, path);
...
int32_t setup_vfio_grp(char  *vfio_container)
{
	...
	NADK_INFO(FW, "\tcontainer device path = %s\n", path);
	if (stat(path, &st) < 0) {
		NADK_ERR(FW, "vfio: LS-container device does not exists\n");
		return NADK_FAILURE;
	}
	...
	if (len == -1) {
		NADK_ERR(FW, "\tvfio: error no iommu_group for device\n");
		NADK_ERR(FW, "\t%s: len = %d, errno = %d\n", path, len, errno);
		return NADK_FAILURE;
	}

	iommu_group_path[len] = 0;
	group_name = basename(iommu_group_path);
	NADK_INFO(FW, "vfio: IOMMU group_name = %s\n", group_name)
		;
	if (sscanf(group_name, "%d", &groupid) != 1) {
		NADK_ERR(FW, "vfio: error reading %s: %m\n", path);
		return NADK_FAILURE;
	}

	NADK_INFO(FW, "vfio: IOMMU group_id = %d\n", groupid);

	/* Check if group already exists */
	for (i = 0; i < VFIO_MAX_GRP; i++) {
		group = &vfio_groups[i];
		if (group->groupid == groupid) {
			NADK_WARN(FW, "groupid already exists %d\n", groupid);
			return NADK_SUCCESS;
		}
	}

	if (NADK_SUCCESS != vfio_set_group(group, groupid)) {
		NADK_ERR(FW, "group setup failure - %d\n", groupid);
		return NADK_FAILURE;
	}
	...
	NADK_INFO(FW, "vfio: Container FD is [0x%X]n", container_device_fd);
...
int64_t vfio_map_mcp_obj(struct vfio_group *group, char *mcp_obj)
{
	...
	if (ret < 0) {
		NADK_ERR(FW, "\tvfio: error getting DEVICE_INFO\n");
		goto MC_FAILURE;
	}
	...
	if (ret < 0) {
		NADK_ERR(FW, "\tvfio: error getting REGION_INFO\n");
		goto MC_FAILURE;
	}

	NADK_INFO(FW, "region offset = %llx  , region size = %llx\n",
