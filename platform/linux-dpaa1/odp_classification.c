...
odp_cos_t odp_cos_create(const char *name)
{
	...
	for (i = 0; i < ODP_COS_MAX_ENTRY; i++) {
		...
		odp_spinlock_lock(&cos->s.lock);
		cos->s.taken = 1;
		cos->s.src_pktio = ODP_PKTIO_INVALID;
		cos->s.queue = ODP_QUEUE_INVALID;
		cos->s.is_default = false;
		cos->s.is_error = false;
		handle = cos->s.cos_id;
		strncpy(cos->s.name, name, ODP_COS_NAME_LEN - 1);
		odp_spinlock_unlock(&cos->s.lock);
...
int odp_cos_queue_set(odp_cos_t cos_id, odp_queue_t queue_id)
{
	...
	if (cos_queue->s.type != ODP_QUEUE_TYPE_PKTIN)
...
static int is_valid_input(size_t val_sz, odp_pmr_term_e term)
{
	switch (term) {
...
static bool is_supported(odp_pmr_term_e term)
{
	switch (term) {
	case ODP_PMR_ETHTYPE_0:
	case ODP_PMR_VLAN_ID_0:
...
odp_pmr_t odp_pmr_create(const odp_pmr_match_t *match)
{
	...
	if (!is_valid_input(match->val_sz, match->term)) {
		ODP_ERR("Invalid field size %d or field type %d\n",
			match->val_sz, match->term);
		return ODP_PMR_INVAL;
	}

	if (!is_supported(match->term)) {
	...
	for (i = 0; i < ODP_PMR_MAX_ENTRY; i++) {
		...
		pmr->s.taken = 1;
		pmr->s.key_size = match->val_sz;
		pmr->s.field = match->term;
		...
		pmr->s.root_id = -1;
		pmr->s.cascade = false;
		copy_keys_bytes(pmr->s.key, (uint8_t *)match->val, match->val_sz);
		copy_keys_bytes(pmr->s.mask, (uint8_t *)match->mask, match->val_sz);
		handle = pmr->s.pmr_id;
...
{
	int loop, j, offset;
	odp_pmr_term_e field, curr_field;
	uint8_t field_size;
			     ...
			     odp_pmr_set_t *pmr_set_id)
{
	...
	for (i = 0; i < ODP_PMRSET_MAX_ENTRY; i++) {
		...
		for (loop = 0; loop < num_terms; loop++) {
			...
			copy_keys_bytes(key, (uint8_t *)terms[loop].val,
				  terms[loop].val_sz);
			copy_keys_bytes(mask, (uint8_t *)terms[loop].mask,
				  terms[loop].val_sz);
